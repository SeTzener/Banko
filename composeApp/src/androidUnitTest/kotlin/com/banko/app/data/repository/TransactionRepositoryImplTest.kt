...
class TransactionRepositoryImplTest {
    ...
    @Test
    fun `should delegate getTransactions to local data source`() = runBlocking {
        val tx = Transaction(
            id = "tx-1",
        ...
        val result = repo.getTransactions(10).first()
        assertEquals(1, result.size)
        assertEquals("tx-1", result[0].id)
    }

    @Test
    fun `should delegate getStoredTransactionCount to local data source`() = runBlocking {
        assertEquals(0, repo.getStoredTransactionCount())
    }

    @Test
    fun `should delegate getOldestTransactions to local data source`() = runBlocking {
        val tx = Transaction(
            id = "tx-1",
            bookingDate = LocalDateTime.parse("2024-01-15T10:30:00"),
            valueDate = LocalDateTime.parse("2024-01-15T12:00:00"),
            ...
            currency = "EUR",
            debtorAccount = null,
            remittanceInformationUnstructured = "Oldest",
            remittanceInformationUnstructuredArray = emptyList(),
            bankTransactionCode = "PMNT",
            internalTransactionId = "int-1",
            creditorName = null,
        ...
        assertEquals(LocalDateTime.parse("2024-01-15T10:30:00"), repo.getOldestTransactions())
    }

    @Test
    fun `should delegate getTransactionsForDateRange to local data source`() = runBlocking {
        ...
        db.bankoDao().upsertTransaction(
            com.banko.app.database.Entities.Transaction(
                id = janTx.id, bookingDate = "2024-01-15T10:30:00", valueDate = "2024-01-15T12:00:00",
                amount = "42.50", currency = "EUR", debtorAccountId = null,
                remittanceInformationUnstructured = "January", remittanceInformationUnstructuredArray = emptyList(),
                bankTransactionCode = "PMNT", internalTransactionId = "int-1",
        ...
        db.bankoDao().upsertTransaction(
            com.banko.app.database.Entities.Transaction(
                id = febTx.id, bookingDate = "2024-02-20T10:30:00", valueDate = "2024-02-20T12:00:00",
                amount = "10.00", currency = "EUR", debtorAccountId = null,
                remittanceInformationUnstructured = "February", remittanceInformationUnstructuredArray = emptyList(),
                bankTransactionCode = "PMNT", internalTransactionId = "int-2",
                creditorName = null, creditorAccountId = null, debtorName = null,
                remittanceInformationStructuredArray = null, note = null, expenseTagId = null
            )
        )

        val janResults = repo.getTransactionsForDateRange(
            LocalDate(2024, 1, 1), LocalDate(2024, 1, 31)
        ).first()
