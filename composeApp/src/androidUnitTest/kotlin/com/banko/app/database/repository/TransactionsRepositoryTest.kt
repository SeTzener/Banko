...
class TransactionsRepositoryTest {
    ...
    @Test
    fun `should upsert and find transaction by id`() = runBlocking {
    ...
    fun `should return oldest transaction date when multiple exist`() = runBlocking {
        ...
        repo.upsertTransaction(newer)
        repo.upsertTransaction(older)

        val result = repo.getOldestTransactions()
        assertEquals(LocalDateTime.parse("2024-01-15T10:30:00"), result)
    }

    @Test
    fun `should return stored transaction count`() = runBlocking {
        ...
        repo.upsertTransaction(tx)
        assertEquals(1, repo.getStoredTransactionCount())
    }

    @Test
    fun `should save note on transaction`() = runBlocking {
        val tx = DaoTransaction(
            id = "tx-1",
            bookingDate = "2024-01-15T10:30:00",
            valueDate = "2024-01-15T12:00:00",
        ...
        repo.upsertTransaction(tx)
        repo.saveNote("tx-1", "Updated note")
    ...
    @Test
    fun `should delete transaction`() = runBlocking {
        val tx = DaoTransaction(
            id = "tx-1",
            bookingDate = "2024-01-15T10:30:00",
            valueDate = "2024-01-15T12:00:00",
        ...
        assertNull(repo.findRawTransactionById("tx-1"))
    }
